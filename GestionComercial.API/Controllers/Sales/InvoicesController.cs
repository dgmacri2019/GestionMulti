...
namespace GestionComercial.API.Controllers.Sales
{
    ...
    public class InvoicesController : ControllerBase
    {
        ...
        public InvoicesController(IWSFEHomologacionService wSFEHomologacionService, IInvoicesNotifier notifierInvoices,
            ...
            IClientsNotifier notifierClients, IArticlesNotifier notifierArticles)
        {
        ...
        public async Task<IActionResult> AnullAsync([FromBody] SaleFilterDto filter)
        {
            try
            {
                ...
                if (saleResponse.Success)
                {
                    ...
                    {
                        if (client.IvaConditionId == 1 || client.IvaConditionId == 2)
                            compTypeId = billing.EmitInvoiceM ? 53 : 3;
                        else
                            compTypeId = 8;
                    }
                    else if (commerceData.IvaConditionId == 2 || commerceData.IvaConditionId == 3)
                    ...
                    if (string.IsNullOrEmpty(invoice.CAE))
                    {
                        InvoiceResponse invoiceAnull = await _invoiceService.FindBySaleIdAsync(sale.Id);
                        if (!invoiceAnull.Success || invoiceAnull.Invoice == null)
                            return BadRequest(new InvoiceResponse { Success = false, Message = invoiceAnull.Message });

                        InvoiceResponse resultAfip = await _wSFEHomologacion.SolicitarCAEAsync(invoice, invoiceAnull.Invoice.Id);
